export function suggestColumnMatch(input: string, destinationHeaders: string[]): { match: string | null; confidence: number } {
    ...
    const aliasMatch = Object.keys(knownAliases).find(k => cleaned.includes(k));
    if (aliasMatch) {
