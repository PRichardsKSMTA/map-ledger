...
export const fetchUserClientAccess = async (
  ...
  logInfo('Fetching user client access (demo mode)', { normalizedEmail });
  logWarn('SQL access disabled; returning fallback user client access data');
  const result = createFallbackUserClientAccess(normalizedEmail);
  logDebug('Returning fallback user client access data', {
