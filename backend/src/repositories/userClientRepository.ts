...
const buildClientMappingSummaryLookup = async (
  ...
  // This query implements the "most recent non-zero" logic to count unique accounts:
  // 1. Get all file records with their net_change values
  // 2. For each unique (client, entity, account), select the most recent GL month with non-zero
  //    net_change, or the most recent GL month if all are zero
  // 3. Count these unique account instances and their mapping status
  const result = await runQuery<MappingSummaryRow>(
    [
      ...
      '  WHERE rn = 1',
      '),',
      // Select the most recent non-zero instance per account, or the most recent if all zero:
      ...
      '    ORDER BY CASE WHEN eam.GL_MONTH = ua.gl_month THEN 0 ELSE 1 END, eam.UPDATED_DTTM DESC',
      '  ) eam',
      ')',
      'SELECT',
      '  client_id,',
