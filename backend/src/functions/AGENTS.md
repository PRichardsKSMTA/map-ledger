...
All `GL_MONTH` and `ACTIVITY_MONTH` columns use the `DATE` data type and must hold the first day of the represented month (e.g., `2024-08-01`). Normalize incoming payloads to this `YYYY-MM-01` pattern before persisting to prevent varchar-based inserts.
