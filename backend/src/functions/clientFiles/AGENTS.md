...
Use these schemas when handling client-file related payloads and database writes. The tables automatically stamp `INSERTED_DTTM` and default `IS_DELETED` to `0`; do not populate `UPDATED_DTTM`, `UPDATED_BY`, `IS_DELETED`, or `DELETED_DTTM` when inserting new rows.
