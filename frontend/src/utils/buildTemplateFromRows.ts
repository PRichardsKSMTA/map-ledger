...
export function buildTemplateFromRows(rows: COARow[], info: TemplateInfo, templateId: string): { template: COATemplate; datapoints: Datapoint[] } {
  ...
  const datapoints: Datapoint[] = rows.map((row, idx) => {
    ...
    return {
      ...
      templateId,
      accountName: String(row['AccountName'] || row['Account'] || row['AccountDescription'] || ''),
      accountDescription: String(row['Description'] || row['AccountDescription'] || ''),
      type: 'Financial',
      ...
      coreGLAccount: String(row['AccountCode'] || row['Code'] || row['Account'] || ''),
